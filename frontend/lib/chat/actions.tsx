...
async function submitUserMessage(sessionId: string, agentId: string, id: string, content: string) {
  ...
  try {
    ...
    if(!res.data.result){
      var content1: string = 'Oops.....! Something error in response'
    }else{
  ...
  // The return part is not necessary unless required by the calling context
  return null
}
...
export const AI = createAI<AIState, UIState>({
  ...
  onGetUIState: async () => {
    ...
    if (session && session.user) {
      ...
      try {
        messages = JSON.parse(aiState.messages);
      } catch (error) {
